...
export default function Settings({ user }) {
  ...
  const [typeEffect, setTypeEffect] = useLocalStorage("layout:typeEffect", true);
  ...
  return <div className='flex flex-col w-100 items-start flex-grow justify-between h-full w-full'>
    <div>
      ...
      {/* layout:typeEffect */}
      <div className="relative flex w-full items-center cursor-pointer my-4">
        <input id="font-sm-checkbox" type="checkbox" checked={typeEffect} className="sr-only peer" />
        <div className="w-11 h-6 bg-gray-600 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600" onClick={(e) => setTypeEffect(!typeEffect)}></div>
        <label htmlFor="font-sm-checkbox" className="ml-2 text-base text-gray-100 dark:text-gray-300">Animação de Escrita</label>
      </div>
